const {Excursion_History} = require('../models/models');
...
class historyController {
  ...
  async getHistory(req, res, next) {
    try {
      const { id } = req.params;
      const history = await Excursion_History.findAll({ where: { userId: id } });
  ...
  async createHistory(req, res, next) {
    try {
      const { id } = req.params;
      const { excursionId, meet, code, manager, name } = req.body;
